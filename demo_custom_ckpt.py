...
def main():
    ...
    parser.add_argument('--weights', type=Path, help='Path to custom checkpoint')
    ...
    if args.weights is not None:
        ckpt_path = args.weights.expanduser()
        ...
        log.info(f'Loaded custom weights from {ckpt_path}')
        log.info(f'Missing keys: {len(missing)}')
        log.info(f'Unexpected keys: {len(unexpected)}')

    else:
        net.load_weights(torch.load(model.model_path, map_location=device, weights_only=True))
        log.info(f'Loaded default weights from {model.model_path}')
    # misc setup
    ...
    if video_path is not None:
        ...
        # Pad sync_frames to exactly 200 frames if slightly short (e.g. 199 due to decoder diff)
        target_sync_frames = seq_cfg.sync_seq_len * 25 // 24  # 192 tokens → 200 frames
        if sync_frames is not None and sync_frames.shape[0] < target_sync_frames:
            pad_len = target_sync_frames - sync_frames.shape[0]
            last_frame = sync_frames[-1:].expand(pad_len, -1, -1, -1).clone()
            sync_frames = torch.cat([sync_frames, last_frame], dim=0)
            log.info(f'Padded sync_frames: {sync_frames.shape[0] - pad_len} → {sync_frames.shape[0]}')
        print("duration =", duration)
        print("clip_frames.shape =", None if clip_frames is None else clip_frames.shape)
        print("sync_frames.shape =", None if sync_frames is None else sync_frames.shape)
        print("seq_cfg.clip_seq_len =", seq_cfg.clip_seq_len)
        print("seq_cfg.sync_seq_len =", seq_cfg.sync_seq_len)
        if mask_away_clip:
            clip_frames = None
        else:
